...
function StempelRaster({ stamps, threshold, stampColor, emptyStyle, preset, useUpload, stampIconUrl }) {
  ...
  const cellSize = Math.min(200/cols, 75/rows) * 0.85
      ...
      {items.map(({cx,cy,filled,num})=>(
        <g key={num}>
          {filled ? (<>
            ...
            <PresetIcon preset={preset} cx={cx} cy={cy} size={r*1.1} color={stampColor} useUpload={useUpload} stampIconUrl={stampIconUrl}/>
          </>) : emptyStyle==='number' ? (<>
            <circle cx={cx} cy={cy} r={r} fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth={1}/>
            <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central" fill="rgba(255,255,255,0.6)" fontSize={r*.85} fontWeight="700">{num}</text>
          </>) : (<>
...
function ApplePreview({ shop, design, stamps, threshold, rewardText, cardName }) {
  ...
  return (
    <div style={{width:210,background:'#1c1c1e',borderRadius:30,padding:'10px 8px 14px',boxShadow:'0 16px 48px rgba(0,0,0,0.45)',margin:'0 auto'}}>
      <div style={{width:65,height:8,background:'#1c1c1e',borderRadius:4,margin:'0 auto 7px'}}/>
      <div style={{background:'#f2f2f7',borderRadius:20,overflow:'hidden',padding:8}}>
        <div style={{borderRadius:11,overflow:'hidden',background:bg,color:fg,boxShadow:'0 4px 14px rgba(0,0,0,0.2)'}}>
          <div style={{display:'flex',alignItems:'center',gap:5,padding:'8px 8px 5px'}}>
            <div style={{width:20,height:20,borderRadius:'50%',background:'rgba(255,255,255,0.95)',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',flexShrink:0}}>
              {logoUrl ? <img src={logoUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/> : <span style={{fontSize:8,fontWeight:700,color:bg}}>SK</span>}
            </div>
            <span style={{fontSize:10,fontWeight:700,flex:1}}>{shop?.name||'Dein Laden'}</span>
            <span style={{fontSize:10,fontWeight:800,color:label}}>{stamps}/{threshold}</span>
          </div>
          {d.walletStyle==='grid' ? (
            ...
            <>
              <div style={{display:'flex',gap:10,padding:'3px 8px 7px'}}>
                <div><div style={{fontSize:7,fontWeight:700,letterSpacing:0.4,color:label,marginBottom:1}}>BELOHNUNG</div><div style={{fontSize:9,fontWeight:600}}>{rewardText||'—'}</div></div>
                <div><div style={{fontSize:7,fontWeight:700,letterSpacing:0.4,color:label,marginBottom:1}}>KARTE</div><div style={{fontSize:9,fontWeight:600}}>{cardName||'—'}</div></div>
              </div>
              {heroUrl && <img src={heroUrl} alt="" style={{width:'100%',height:40,objectFit:'cover',display:'block'}}/>}
            </>
          )}
...
function GooglePreview({ shop, stamps, threshold, rewardText, cardName }) {
  ...
  const heroUrl = shop?.heroImageUrl||null
  return (
    <div style={{borderRadius:14,overflow:'hidden',background:bg,color:fg,boxShadow:'0 6px 20px rgba(0,0,0,0.15)'}}>
      <div style={{display:'flex',alignItems:'center',gap:9,padding:'12px 12px 7px'}}>
        <div style={{width:30,height:30,borderRadius:'50%',background:'white',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',flexShrink:0}}>
          {logoUrl ? <img src={logoUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/> : <span style={{color:'#3C3489',fontWeight:700,fontSize:11}}>SK</span>}
        </div>
        <div style={{fontSize:13,fontWeight:600}}>{shop?.name||'Dein Laden'}</div>
      </div>
      <div style={{display:'flex',gap:14,padding:'0 12px 10px'}}>
        <div><div style={{fontSize:9,fontWeight:700,letterSpacing:0.5,color:label,marginBottom:2}}>STEMPEL</div><div style={{fontSize:12,fontWeight:600}}>{stamps}/{threshold}</div></div>
        <div><div style={{fontSize:9,fontWeight:700,letterSpacing:0.5,color:label,marginBottom:2}}>BELOHNUNG</div><div style={{fontSize:12,fontWeight:600}}>{rewardText||'—'}</div></div>
        <div><div style={{fontSize:9,fontWeight:700,letterSpacing:0.5,color:label,marginBottom:2}}>KARTE</div><div style={{fontSize:12,fontWeight:600}}>{cardName||'—'}</div></div>
      </div>
      {heroUrl && <img src={heroUrl} alt="" style={{width:'100%',height:75,objectFit:'cover',display:'block'}}/>}
      <div style={{display:'flex',justifyContent:'center',padding:10,background:'rgba(255,255,255,0.08)'}}><MockQR size={64}/></div>
    </div>
  )
}
...
function DesignPanel({ design, onChange, cardId=null }) {
  ...
  async function uploadIcon(e) {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        const endpoint = cardId ? `/api/shop/cards/${cardId}/stamp-icon` : '/api/shop/stamp-icon'
  ...
  return (
    <div>
      <div style={dp.block}>
        <div style={dp.title}>Wallet-Stil</div>
        <div style={dp.row2}>
          {[{val:'number',label:'🔢 Zahlen',desc:'Klassisch'},{val:'grid',label:'🟡 Raster',desc:'Stempel als Grid'}].map(({val,label,desc})=>(
      ...
      <div style={dp.block}>
        <div style={dp.title}>Stempel-Icon</div>
        <div style={dp.row2}>
          {[{val:'preset',label:'🎨 Vorlage'},{val:'upload',label:'📁 Eigenes Bild'}].map(({val,label})=>(
      ...
      <div style={dp.block}>
        <div style={dp.title}>Stempel-Farbe</div>
        <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
          {['#6F4E37','#FFD700','#E74C3C','#2ECC71','#3498DB','#9B59B6','#1A1A1A','#FFFFFF'].map(c=>(
            <div key={c} style={{width:26,height:26,borderRadius:'50%',background:c,cursor:'pointer',flexShrink:0,transition:'transform 0.1s',
      ...
      <div style={dp.block}>
        <div style={dp.title}>Leere Stempel</div>
        <div style={dp.row2}>
...
const dp = {
  ...
  title: {fontSize:11,fontWeight:800,color:'#888',marginBottom:8,textTransform:'uppercase',letterSpacing:0.8},
  ...
  uploadBtn: {padding:'6px 14px',borderRadius:7,border:'2px solid #3C3489',background:'white',color:'#3C3489',fontSize:12,fontWeight:600,cursor:'pointer'},
}
...
export default function Karten() {
  ...
  useEffect(()=>{
    loadCards()
    api.get('/api/shop/me').then(r=>setShop(r.data))
  },[])
  ...
  const editPreviewStamps = Math.floor(editThreshold/2)
  ...
  if (mode==='list') return (
    <div>
      <div style={s.header}>
        <div>
          <h1 style={s.title}>Karten</h1>
          <p style={s.subtitle}>Deine Stempelkarten</p>
        </div>
  ...
  if (mode==='create') return (
    <div>
      <div style={s.header}>
        <div>
          <h1 style={s.title}>Neue Karte erstellen</h1>
          <p style={s.subtitle}>Infos + Design in einem Schritt</p>
        </div>
      ...
      <div style={s.createGrid}>
        ...
        <div style={s.panel}>
          ...
          <div style={s.field}>
            ...
            <input style={s.input} type="number" min="1" max="100"
              value={form.rewardThreshold} onChange={e=>setForm({...form,rewardThreshold:e.target.value})}/>
          </div>

          <div style={{marginTop:16,marginBottom:16}}>
            <div style={s.sliderLabel}>Vorschau: {previewStamps}/{threshold} Stempel</div>
        ...
        {/* Spalte 2: Design */}
        <div style={s.panel}>
          <div style={s.panelTitle}>Stempel-Design</div>
  ...
  if (mode==='edit') return (
    <div>
      <div style={s.header}>
        <div>
          <h1 style={s.title}>{editCard.name}</h1>
          <p style={s.subtitle}>Design bearbeiten</p>
        </div>
      ...
      <div style={s.editGrid}>
        ...
        <div style={s.panel}>
          ...
          <button style={{...s.btnCreate,...(saved?{background:'#2C5F2E'}:{})}} onClick={saveEditDesign} disabled={loading}>
            {saved?'✓ Gespeichert!':loading?'Speichere…':'💾 Design speichern'}
          </button>
        </div>
        ...
        <div>
          <div style={s.previewLabel}>🍎 Apple Wallet</div>
          <ApplePreview shop={shop} design={editDesign} stamps={editPreviewStamps}
            threshold={editThreshold} rewardText={editCard.rewardText} cardName={editCard.name}/>
          <div style={{...s.previewLabel,marginTop:20}}>🤖 Google Wallet</div>
          <GooglePreview shop={shop} stamps={editPreviewStamps} threshold={editThreshold}
            rewardText={editCard.rewardText} cardName={editCard.name}/>
          <p style={s.hint}>Farben & Logo aus Profil-Einstellungen</p>
        </div>
      </div>
    </div>
...
const s = {
  ...
  panel: {background:'white',borderRadius:12,padding:'20px',boxShadow:'0 2px 8px rgba(0,0,0,0.06)'},
  ...
  sliderLabel: {fontSize:12,fontWeight:600,color:'#3C3489',marginBottom:6},
  slider: {width:'100%',accentColor:'#3C3489'},
  btnCreate: {width:'100%',marginTop:4,padding:13,borderRadius:10,border:'none',background:'#3C3489',color:'white',fontSize:14,fontWeight:700,cursor:'pointer',transition:'background 0.2s'},
